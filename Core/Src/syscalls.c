...
/**
  * @brief Write a wide character to a file (newlib implementation)
  * @param ptr Pointer to reentrancy structure
  * @param wc Wide character to write
  * @param file FILE structure pointer
  * @retval wint_t The character written or WEOF on error
  */
wint_t _fputwc_r(struct _reent *ptr, wchar_t wc, FILE *file)
{
  (void)ptr;
  (void)file;

  // For embedded systems, we can convert wide char to regular char
  // and output it using the standard putchar function
  if (wc <= 0xFF) {
    __io_putchar((char)wc);
    return (wint_t)wc;
  }

  // For characters outside ASCII range, return error
  return EOF;
}

#endif // !defined(__PICOLIBC__) && defined(__NEWLIB__)

/**
  * @brief Abort function to check for debugger and act accordingly
  */
#include "core_cm7.h"
...
void abort(void)
{
  ...
  {
    // Debugger is not connected, reboot the system
    NVIC_SystemReset();
  }
  while(1);
}
